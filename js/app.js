...
import {
  bindModulePipelineHost,
  casePackWarnings,
  checkCasePack,
  checkModulePayload,
  firstCasePackProblem,
  TABLE_HIGHLIGHT_KINDS,
  applyHousePurchaseProjectionToModule,
  applyMortgageProjectionToModule,
  applyNetRetirementProjectionToModule,
  applyNormalizedPayloadToModule,
  applyPensionProjectionToModule,
  clearGeneratedForVideoSummary,
  cloneSessionValue,
  createBlankModule,
  ensureGenerated,
  ensureModuleUi,
  getDefaultLoanKindForSource,
  getLoanEngineInputs,
  getLoanEngineSource,
  injectAutoPensionSummarySentences,
  normalizeDevPanelPayload,
  normalizePayload,
  normalizeTableHighlightAnchor,
  nowIso,
  preflightGeneratedPayload,
  setLoanEngineInputs
} from './module_pipeline.js';

/**
 * Hand the pipeline the three things it cannot do on its own.
 *
 * Bound here at module load rather than inside `initApp`, because the pipeline
 * is inert until this runs and a projection that quietly stops being recorded
 * is the kind of failure nothing reports.
 */
bindModulePipelineHost({
  recordProjection: (moduleId, record) => {
    appState.lastValidProjectionByModuleId.set(moduleId, record);
  },
  resolveMortgageScenarioId: (moduleId) => getMortgageScenarioForModule(moduleId),
  resetAssumptionsEditorState: (moduleId) => resetAssumptionsEditorState(moduleId)
});
...
/**
 * Already-checked payloads, added as new modules or not at all.
 *
 * The checking happens before this is called -- a pack whose fourth module is
 * wrong must not leave three behind for someone to delete by hand before they
 * can try again, which is the failure that makes loading a whole call worse
 * than pasting one module at a time.
 *
...
async function applyModuleBatchInternal(payloads) {
  ...
  try {
    ...
    const moduleIds = payloads.map((normalizedPayload) => {
      const module = createBlankModule();
      appState.session.modules.push(module);
      appState.session.order.push(module.id);
      applyNormalizedPayloadToModule(module, normalizedPayload);
    ...
    try {
      ...
      // Browser storage is finite and a whole call is a lot of JSON. Saying so
      // beats leaving modules on screen that will not survive a reload.
      throw new Error(`Could not save the call (${error?.message || 'storage is full'}); nothing was added.`);
    }
...
/**
 * What is in the box, checked but not yet applied.
 *
 * A pack and a loose paste are checked by the same per-module function; the
 * only difference is that a pack also has a version, a client name and a rule
 * about unknown keys. Nothing here touches the session.
 */
function reviewPastedModules() {
  let extracted;
  try {
    extracted = extractModulePayloadsFromEditorText(ui.devPayloadInput.value || '');
  } catch (error) {
    return { ok: false, problem: error?.message || 'Invalid JSON (check quotes)', warnings: [] };
  }

  if (extracted.pack) {
    const result = checkCasePack(extracted.pack);
    return {
      ok: result.ok,
      isPack: true,
      clientName: result.clientName,
      problem: firstCasePackProblem(result),
      warnings: casePackWarnings(result),
      payloads: result.modules.map((entry) => entry.payload)
    };
  }

  const total = extracted.payloads.length;
  const rows = extracted.payloads.map((payload, index) => (
  ...
  return {
    ...
    isPack: false,
    clientName: '',
    problem: failed ? `Module ${failed.number} of ${failed.total} (${failed.title}): ${failed.error}` : '',
    warnings: rows.flatMap((row) => row.warnings.map((warning) => (
      `Module ${row.number} of ${row.total} (${row.title}): ${warning}`
    ))),
    payloads: rows.map((row) => row.payload)
  };
}

function reportPastedModuleReview(review) {
  if (!review.ok) {
    renderDevPayloadWarnings(review.warnings, { errorMessage: review.problem });
    showToast(review.problem, 'error');
    return false;
  }

  renderDevPayloadWarnings(review.warnings);
  if (review.warnings.length > 0) {
    console.warn('[CallCanvas][DevPayload] auto-repairs applied', review.warnings);
  }
  return true;
}

function describeRepairs(warnings) {
  return warnings.length > 0
    ? ` with ${warnings.length} auto-repair${warnings.length === 1 ? '' : 's'}`
    : '';
}
...
async function applyModuleBatchFromEditor() {
  ...
  const review = reviewPastedModules();
  if (!reportPastedModuleReview(review)) {
    return;
  }

  try {
    const { moduleIds } = await applyModuleBatchInternal(review.payloads);
    renderDevPayloadWarnings(review.warnings);
    const moduleCount = `${moduleIds.length} module${moduleIds.length === 1 ? '' : 's'}`;
    showToast(`Added ${moduleCount}${describeRepairs(review.warnings)}.`);
  } catch (error) {
    renderDevPayloadWarnings(review.warnings, {
...
/**
 * A pack, opened as the call it describes.
 *
 * Confirmed the same way starting a new call by hand is, because it throws
 * away the open one just as thoroughly.
 */
async function startNewCallFromPackInEditor() {
  if (!ui.devPayloadInput || runtimeConfig.readOnly) {
    return;
  }

  const review = reviewPastedModules();
  if (!review.isPack) {
    const problem = review.ok
      ? 'That is a module paste, not a case pack. Use Add All Modules, or add casePackVersion and clientName.'
      : review.problem;
    renderDevPayloadWarnings(review.warnings, { errorMessage: problem });
  ...
  if (hasModules() && !window.confirm('Start a new call from this pack? Unsaved changes will be lost.')) {
    return;
  }
  ...
  try {
    await replaceSession(newSession(review.clientName), { markClean: true });
    const { moduleIds } = await applyModuleBatchInternal(review.payloads);
    renderDevPayloadWarnings(review.warnings);
    const moduleCount = `${moduleIds.length} module${moduleIds.length === 1 ? '' : 's'}`;
    showToast(`Started a new call for ${review.clientName}: ${moduleCount}${describeRepairs(review.warnings)}.`);
  } catch (error) {
    appState.session = previousSession;
    await rerenderAfterPayloadRollback(previousMode);
    const message = error?.message || 'Failed to start a call from that pack.';
    renderDevPayloadWarnings(review.warnings, { errorMessage: message });
    showToast(message, 'error');
  }
}

/** Packs are too long to paste comfortably, so they can be opened from disk. */
async function loadCasePackFileIntoEditor(file) {
  if (!file || !ui.devPayloadInput) {
    return;
  }

  try {
    ui.devPayloadInput.value = await file.text();
    const review = reviewPastedModules();
    const loaded = review.isPack
      ? `Loaded ${file.name}: a case pack for ${review.clientName || 'an unnamed client'}.`
      : `Loaded ${file.name}.`;
    if (!review.ok) {
      renderDevPayloadWarnings(review.warnings, { errorMessage: review.problem });
      showToast(review.problem, 'error');
    ...
    renderDevPayloadWarnings(review.warnings);
    showToast(`${loaded} ${review.payloads.length} module${review.payloads.length === 1 ? '' : 's'} ready.`);
  } catch (error) {
    const message = error?.message || 'Could not read that file.';
...
function bindEvents() {
  ...
  if (runtimeConfig.allowDevPanel && ui.devNewCallFromPackBtn) {
    ui.devNewCallFromPackBtn.addEventListener('click', async () => {
      await startNewCallFromPackInEditor();
    });
  }

  if (runtimeConfig.allowDevPanel && ui.devLoadPackFileBtn && ui.devPackFileInput) {
    ui.devLoadPackFileBtn.addEventListener('click', () => {
      ui.devPackFileInput.click();
    });
    ui.devPackFileInput.addEventListener('change', async (event) => {
      const [file] = event.target.files || [];
      // Cleared so choosing the same file twice in a row still fires.
      event.target.value = '';
      await loadCasePackFileIntoEditor(file);
    });
  }
