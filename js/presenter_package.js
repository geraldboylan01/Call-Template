...
export function compilePresentation(pkg, catalogue, script = '') {
  ...
  assert(!script.includes('[→'), 'script.md must contain spoken text, without presenter cues.');
  ...
  const compiled = steps.map(step => {
    keys(step, ['id', 'label', 'operations', 'rationale', 'edit'], 'step');
    if (step.edit !== undefined) {
      keys(step.edit, ['shot', 'reason'], `${step.id} edit`);
      assert(['screen', 'presenter', 'hold'].includes(step.edit.shot), `${step.id}: edit shot must be screen, presenter or hold.`);
      assert(typeof step.edit.reason === 'string' && step.edit.reason.trim(), `${step.id}: explain the editorial suggestion.`);
    }
    ...
    for (const op of step.operations) {
      keys(op, ['action', 'target', 'scenarioId', 'emphasis'], `${step.id} operation`);
      ...
      if ('emphasis' in op) {
        assert(op.action === 'focus', `${step.id}: emphasis belongs only to FOCUS.`);
        assert(['spotlight', 'underline', 'point'].includes(op.emphasis), `${step.id}: unsupported emphasis ${op.emphasis}.`);
        assert(op.emphasis !== 'point' || target.ref.type === 'chart-point', `${step.id}: point emphasis requires a chart point.`);
      }
      if (op.action === 'state') {
      ...
      visual = { targetId: target.id, moduleKey: target.moduleKey, action: op.action === 'state' ? 'frame' : op.action, emphasis: op.emphasis };
    }
  ...
  for (const claim of review?.claims || []) {
    ...
    for (const fact of claim.facts || []) {
      const target = targets.get(fact.targetId);
      if (!target || JSON.stringify(target.value) !== JSON.stringify(fact.value)) narrativeIssues.push(`Source value changed: ${fact.targetId}`);
    }
