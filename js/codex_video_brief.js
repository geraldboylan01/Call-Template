...
export function buildCodexVideoBrief({
  ...
  return {
    ...
    delivery: {
      ...
      presenterRecorder: {
        mode: 'browser-native local presenter recording',
        camera: 'getUserMedia camera and microphone preview inside the presenter-safe zone',
        capture: 'getDisplayMedia current tab/window capture recorded with MediaRecorder',
        saveFormat: 'local WebM download',
        stopControls: 'S and Escape stop recording; an emergency stop button may reveal on hover but must hide before stopping',
        mirrorCameraByDefault: true,
        faceTracking: 'local MediaPipe BlazeFace crop-following with adaptive zoom',
        faceTrackingFallback: 'mirrored centred crop if local MediaPipe assets are unavailable',
        microphoneRequired: true,
        microphoneCapture: 'verify a live getUserMedia microphone track and mix it into the MediaRecorder output',
...
export function buildCodexVideoInstruction(brief) {
  ...
  return `You are the Planeir Codex video director. Create one bespoke visual-first 16:9 HTML/CSS/JS browser recording page from the approved local brief below. Do not call any external AI service or add an API key. Work only in ${outputDirectory}. Create index.html, scoped CSS/JS as needed, storyboard.md, source-brief.json, and quality-review.md.\n\nTell the story with a 3-5 beat sequence when that is enough, or add extra manually navigable scenes when the data needs more room. The page must support presenter-led step-through with keyboard/buttons, and autoplay must be optional rather than the only way to use it. Progress indicators must accurately represent the current scene in manual mode, including the final scene. Use charts, comparison, flow, and spatial motion to communicate meaning; keep on-screen copy short. Preserve a clean presenter-safe right third through every scene, unless the brief explicitly requests a mirrored composition. Use only approved data from the brief; public-facing scenes must omit operational details and access information. Make the output recording-ready in a browser at 1920x1080 and responsive at 1280x720. Include a local-only presenter recorder mode: the presenter camera preview must sit inside the safe zone, Start camera must use getUserMedia, Start recording must use getDisplayMedia plus MediaRecorder, recording must save as a local WebM download, and clean fullscreen capture must hide setup/playback controls while keeping keyboard scene navigation available. The presenter camera must be mirrored by default in preview and final recording. Add free local smart-camera framing using vendored MediaPipe BlazeFace assets where possible: follow the highest-confidence detected face at roughly 8-12fps, smooth the crop so head movement does not jitter, adapt zoom to keep a head-and-shoulders framing inside the presenter-safe zone, and fall back to a mirrored centred crop if MediaPipe assets cannot load. Verify a live microphone track from getUserMedia before recording; if no live microphone is available, do not create a silent recording. Mix microphone audio into the final MediaRecorder stream and make the UI state clearly say whether camera + mic are ready. Stopping must be easy in fullscreen: S and Escape must stop recording and reveal the save panel, browser stop-sharing must also finish cleanly, and any emergency on-screen stop button must stay hidden unless invoked and hide itself before stopping.\n\nDesign for YouTube retention, not decoration: the first scene must immediately deliver the title/topic promise; each later scene must introduce a visual change, comparison, reveal, question, or motion beat before the screen feels static. Avoid the feel of a PowerPoint with animated bullets: build a small reusable visual system and move shared elements across scenes wherever possible, such as the same money token, path, lens, decision fork, reserve bucket, or risk route. When adjacent scenes explain the same idea, create a visible handoff by reusing colour, shape, position, icon, path, or motion so the sequence feels designed rather than slide-by-slide. Every line, arrow, icon, or path must have an obvious meaning; replace ambiguous decorative marks with clearer semantic visuals. Identify the underlying problem before presenting the route: if one option is being considered because it might solve two different problems, separate those problems visually before showing the solution. When the client question is an immediate decision, such as buying now versus buying later, show that decision before derived implementation mechanics like reserve sizing or investment buckets. Use purposeful continuous motion when it clarifies the story, such as flowing money paths, risk pulses, timeline sweeps, route highlighting, or chart emphasis. Avoid visual noise, tiny labels, dense text, and motion in the presenter-safe zone.\n\nTreat source data with enough nuance to be truthful. If the brief includes a study, table, return series, fixed/risk-free rates, average returns, and worst-case/downside values, do not compress it into a single worst-return graphic. Name the source and period when supplied, show the risk-free or fixed-deposit comparison, show average-return potential and downside risk, and make clear whether values are illustrative, historical, nominal, before fees/tax/inflation, or based on a specific starting amount. If a scene shows a derived figure, include the formula and planning rationale when supplied by the brief, rather than only the final number. If a middle-ground route is shown, define the two endpoints from the real client decision in the brief, not from arbitrary labels.\n\nBefore finishing, run a visible quality loop and document it in quality-review.md. Review every scene at 1920x1080 and 1280x720, preferably by opening the page or using a local browser/screenshot tool when available. Fix and explicitly check: duplicate or colliding Planeir marks; text overlapping lines, charts, cards, controls, or other text; ambiguous decorative marks that could be misread as charts, arrows, or decision paths; presenter camera fit inside the safe zone; mirrored camera output; face remains centred during modest head movement; adaptive zoom remains stable without pulsing; lost-face fallback returns to a centred crop; no external AI/API calls for camera tracking; live microphone track is present before recording; recorded WebM contains microphone audio; recorder/setup controls hidden during capture; fullscreen stop controls work without forcing visible UI into the recording; metric rows and narrative copy sharing the same visual area; copy or important motion entering the presenter-safe third; illegible text at 1280x720; static dead time after the initial reveal; control UI covering recording content; manual step-through and progress indicators including the last scene; scene-to-scene continuity where one idea spans adjacent scenes; reuse of moving visual elements instead of slide-by-slide rebuilds; scene order identifying the underlying problem before solution mechanics; scene order matching the client's immediate decision before derived implementation details; and any visual mismatch between the brief and the first 30 seconds. Do not pass the quality loop if any value, label, paragraph, icon, line, or card visibly intersects another element. If a browser preview tool is unavailable, do deterministic static/layout checks and state that limitation in quality-review.md.\n\nAttach any listed module images separately before using them. Do not invent client facts or financial figures.\n\nCODEX VIDEO BRIEF v${CODEX_VIDEO_BRIEF_VERSION}:\n${JSON.stringify(brief, null, 2)}`;
}
