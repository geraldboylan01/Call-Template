...
const directory = resolve(process.env.AI_LED_OUTPUT || 'diagnostics/ai-led-simplified', `r${repetition}`, test.id);
...
const summary = { id: test.id, repetition, commit: execFileSync('git', ['rev-parse', 'HEAD'], { encoding: 'utf8' }).trim(),
  ...
  scope: 'Synthetic planning laboratory; no production changes, browser, speech, or deployment',
...
for (arm of orders[(repetition - 1) % orders.length]) {
  ...
  const record = { arm, id: test.id, repetition, turns: test.turns, expected: test.expected, expectsReady: test.ready !== false,
