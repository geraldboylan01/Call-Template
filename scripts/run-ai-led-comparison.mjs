...
const output = process.env.AI_LED_OUTPUT || 'diagnostics/ai-led-comparison-main-v1';
...
async function worker() {
  for (;;) {
    ...
    try { const saved = JSON.parse(await readFile(`${path}/summary.json`, 'utf8'));
      if (saved.arms.length === 4) { results.push(saved); continue; }
    } catch {}
    const child = spawn(process.execPath, ['--env-file=.env.local', 'scripts/compare-ai-led-recovery.mjs'], {
