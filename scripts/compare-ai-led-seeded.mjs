...
const directory = resolve(process.env.AI_LED_OUTPUT || 'diagnostics/ai-led-comparison', `r${repetition}`, seed.id);
...
async function simpleLoop(arm, operation) {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    if (attempt && arm === 'rethink') body.priorAuditFindings = {
      rejectedProposal: actual.plannerFacingSnapshot(snapshot), latestIndependentCriticism: verification,
...
const orders = [['full_repair', 'option2', 'rethink', 'fresh'], ['fresh', 'rethink', 'option2', 'full_repair']];
const summary = { id: seed.id, originalCaseId: test.id, injectedManifest: seed.manifest, repetition, commit: execFileSync('git', ['rev-parse', 'HEAD'], { encoding: 'utf8' }).trim(),
  sourceHash: hash(source), scope: 'Injected incorrect first proposal; real independent verifier and recovery; zero latency/tokens for synthetic first proposal; no production changes',
...
for (arm of orders[(repetition - 1) % orders.length]) {
  ...
  const operation = { id: `ai-led-${test.id}-${repetition}-${arm}`, deadlineAt: simulatedStarted + 90000,
    callAllowance: ['rethink', 'fresh'].includes(arm) ? 4 : 7, callsUsed: 0, controller: new AbortController() };
  const record = { arm, id: seed.id, originalCaseId: test.id, repetition, turns: test.turns, expected: test.expected,
    providerCalls: calls, checks: [], latestCandidate: null };
  try {
    record.result = ['rethink', 'fresh'].includes(arm) ? await simpleLoop(arm, operation)
