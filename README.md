...
Run its current milestone gate from the repository root:
...
- Way 2, the API: `POST https://api.planeir.ie/api/agent/applications/check` returns the write-up, precise warnings and questions still worth asking, and stores nothing. `POST /api/agent/applications` needs `person`, both `consent` flags and `application`; it files a request (`agent_application_requests`, migration 0018) and emails the person one button, **Confirm my application**, linking to `/apply/confirm/#t=...`. The person already reviewed the answers with their assistant (the check summary), so there is no second review: the page confirms as it opens and thanks them. Only then is a lead and client created (`source = 'agent-api'`). Pressing the button again shows the same thank-you. Every request, confirmed or not, is deleted by the hourly cron 7 days after it arrived.
