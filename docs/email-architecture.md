...
- Lead scheduling replies from `/app/leads.html` create a Zoom meeting first, use the Zoom join URL in the email and `planeir-call.ics` calendar invite, include 48-hour accept/decline links, delete declined or expired unaccepted Zoom meetings, and send through the same Resend sender. Advisor copies go to `LEAD_ADVISOR_COPY_TO`, falling back to `LEAD_NOTIFICATION_TO`.
