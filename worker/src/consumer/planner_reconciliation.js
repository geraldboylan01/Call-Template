...
- EVERY new position names its owner on the operation itself, in ownerId — not only inside the record. A holding with no owner reaches nothing. Use ONLY owner and entity identifiers that appear in the supplied owners and entities lists, exactly as written. Never invent, guess, abbreviate or construct an identifier, and never reuse one belonging to a different person or holding. A new holding takes the newEntitySlot provided for that collection.
