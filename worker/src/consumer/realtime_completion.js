...
export const REALTIME_COMPLETION_OUTRO = 'Thanks very much for your time today. Your modules are ready, and I’m taking you to them now.';
...
const AFFIRMATIVE_TOKENS = new Set([
  ...
  'modules', 'now', 'so', 'lets', 'just', 'move', 'on', 'im', 'happy', 'with',
  'and', 'absolutely', 'sure'
]);
...
export function buildVoiceConfirmationSummary({
  ...
  const names = analyses.map((item) => item.label).filter(Boolean).slice(0, 3);
  const moduleText = names.length === 1
    ? names[0]
    : names.length === 2
      ? `${names[0]} and ${names[1]}`
  ...
  const pensionText = statePensionRule
    ...
    : '';
  return `${narrativeSummary ? `To confirm, ${narrativeSummary.replace(/[.?!]+$/, '')}. ` : ''}I’m ready to generate ${moduleText || 'the selected modules'} using the assumptions shown.${assumptionsText}${pensionText} Does that all sound correct, and would you like me to generate those modules now?`;
}
